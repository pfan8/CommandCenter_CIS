...
void BotConfig::readConfigFile()
{
	...
	// Parse the SC2API Options
	if (j.count("SC2API") && j["SC2API"].is_object())
	{
		const json & sc2api = j["SC2API"];
		JSONTools::ReadString("BotRace", sc2api, BotRace);
		JSONTools::ReadInt("EnemyDifficulty", sc2api, EnemyDifficulty);
		JSONTools::ReadString("EnemyRace", sc2api, EnemyRace);
		JSONTools::ReadString("MapFile", sc2api, MapName);
		JSONTools::ReadInt("StepSize", sc2api, StepSize);
	}
