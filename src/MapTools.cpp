...
bool MapTools::canBuildTypeAtPosition(int x, int y, const sc2::UnitTypeID type) const
{
	return m_bot.Query()->Placement(Util::UnitTypeIDToAbilityID(type), sc2::Point2D{ static_cast<float>(x), static_cast<float>(y) });
}
...
bool MapTools::IsAnyTileAdjacentToTileType(const sc2::Point2DI p, const MapTileType tile_type, const sc2::UnitTypeID building_type) const
{
	...
	if (endx < 0 || endy < 0 || startx > m_bot.Map().width() || starty > m_bot.Map().height())
	{
		return false;
	}
...
void MapTools::draw() const
{
    ...
    for (int x = 1; x < m_bot.Map().width(); ++x)
    {
        for (int y = 1; y < m_bot.Map().height(); y++)
        {
