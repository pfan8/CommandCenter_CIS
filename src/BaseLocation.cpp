    ...
    , m_bottom               (std::numeric_limits<CCPositionType>::max())
{
    ...
    if (!isStartLocation())
    {
        ...
        for (auto & tile : getClosestTiles())
        {
            ...
            if (m_bot.Map().canBuildTypeAtPosition(buildTile.x, buildTile.y, depot.getAPIUnitType()))
            {
...
bool BaseLocation::isInResourceBox(int tileX, int tileY) const
{
	...
	/*std::cout << "base:" << m_baseID << ":" << m_left << " + " << m_right
		<< ";" << m_bottom << " + " << m_top << std::endl;
...
void BaseLocation::draw()
{
	...
	const sc2::Point2D base_location = m_bot.Bases().getPlayerStartingBaseLocation(player)->getPosition();
	ss << "\n";
	ss << "base_location_x:" << static_cast<int>(base_location.x) << "\n";
	ss << "base_location_y:" << static_cast<int>(base_location.y);
