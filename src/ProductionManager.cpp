...
void ProductionManager::create(const Unit & producer, BuildOrderItem & item)
{
    ...
    if (item.type.isBuilding())
    {
		...
		else if (item.type.getUnitType().isTechLab())
		{
			producer.buildTech();
		}
        else
...
bool ProductionManager::canMakeNow(const Unit & producer, const MetaType & type)
{
    ...
    else
    {
        ...
        {
			if (available_ability.ability_id == sc2::ABILITY_ID::BUILD_TECHLAB)
			{
				if (MetaTypeAbility == sc2::ABILITY_ID::BUILD_TECHLAB_BARRACKS ||
					MetaTypeAbility == sc2::ABILITY_ID::BUILD_TECHLAB_FACTORY ||
					MetaTypeAbility == sc2::ABILITY_ID::BUILD_TECHLAB_STARPORT )
				{
					return true;
				}
			}
